...
void BoostGraph::setConnected(const Edge &e, bool connected)
{
    if(connected) {
        add_edge(e.first, e.second, m_graph);
    } else {
...
NodeMapping BoostGraph::subgraphIsoOne(BoostGraph *needle)
{
...
bool BoostGraph::subgraphIsoHasOne(vector<BoostGraph> needle)
{
    int count = 0;
...
vector<NodeMapping> BoostGraph::subgraphIsoAll(const BoostGraph *needle)
{
...
int BoostGraph::subgraphIsoCountAll(const BoostGraph *needle)
{
    int count = 0;
